...
@Component({selector: 'app-principal',templateUrl: './principal.component.html',
...
export class PrincipalComponent implements OnInit {
  ...
  this.toastr.success('Registro exitoso', 'Estudiante registrado',{positionClass:'toast-bottom-right'});
      this.usuario = {} as estudiante;this.router.navigate(['/list-estudiante']).catch(error =>{
    console.log(error);})})}
  ...
  editar(id:number){this.http.get<estudiante>("https://frozen-meadow-48728.herokuapp.com/uno/"+id)
  ...
  eliminar(id:number): void {
    if (confirm('¿Eliminar estudiante?')) {
      ...
      .subscribe(response => {alert('Estudiante eliminado');
         window.location.reload();},error => {console.log(error);}); }}
